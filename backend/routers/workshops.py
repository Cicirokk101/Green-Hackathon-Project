...
from models import User, Workshop, WorkshopMembership
...
async def list_workshops(
    ...
    for w in workshops:
        ...
        attending = await _is_attending(db, w.id, user.id)
        items.append(_build_workshop_out(w, taken, attending, user.id))
...
async def create_workshop(
    ...
    return _build_workshop_out(workshop, 0, False, user.id)
...
async def get_workshop(
    ...
    attending = await _is_attending(db, workshop_id, user.id)
    return _build_workshop_out(workshop, taken, attending, user.id)
...
async def leave_workshop(
    ...
    await db.execute(
        delete(WorkshopMembership).where(
            WorkshopMembership.workshop_id == workshop_id,
            WorkshopMembership.user_id == user.id,
        )
    )
    await db.commit()
