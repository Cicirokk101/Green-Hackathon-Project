...
const SEATS_OPTIONS = ["4", "6", "8", "10", "12", "16", "20"];
...
function PreviewCard({ f }: { f: WorkshopForm }) {
  ...
  const icon = CAT_ICON[f.cat] || "bulb";
  const seatsLeft = Number(f.seats) || 8;
  ...
  return (
    ...
    >
      ...
      >
        <Icon name={icon} size={36} color="#fff" sw={1.4} />
      </div>

      <div style={{ minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
          <Eyebrow color={cat.fg}>{f.cat}</Eyebrow>
          <span style={{ fontSize: 11.5, color: K.faint, fontWeight: 600 }}>· {f.level}</span>
        </div>
        <h4
          style={{
            fontFamily: K.serif,
      ...
      <div style={{ textAlign: "right", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 10, flexShrink: 0 }}>
        <div style={{ fontSize: 12.5, color: K.muted, fontWeight: 700 }}>{seatsLeft} seats left</div>
        <Button variant="primary" disabled>Reserve a seat</Button>
      </div>
    </div>
  );
}

export function StartWorkshopPage() {
  ...
  const set = (k: keyof WorkshopForm) =>
    (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
      setF({ ...f, [k]: e.target.value });

  async function handlePost() {
    if (!f.skill || !f.when || !f.place) return;
    setSubmitting(true);
  ...
  return (
    <div style={{ maxWidth: 1080, margin: "0 auto", padding: "40px 36px 64px" }}>
      ...
      <h1 style={{ fontFamily: K.serif, fontSize: 38, fontWeight: 700, margin: "0 0 6px" }}>Host a workshop</h1>
      <p style={{ fontSize: 16, color: K.muted, margin: "0 0 32px", maxWidth: 560 }}>
        Teach something you know. It earns you karma and leaves neighbors better off.
      </p>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 40, alignItems: "start" }}>
        <div style={{ background: "#fff", borderRadius: 22, padding: "30px 32px", boxShadow: K.shadow }}>
          <Field label="What will you teach?">
            <CatPicker value={f.cat} onChange={(c) => setF({ ...f, cat: c })} />
          </Field>

          <Field label="Workshop title" hint="Plain and specific — e.g. 'Sourdough basics' or 'Bike tune-up clinic'">
            <TextInput
              value={f.skill}
              onChange={set("skill")}
              placeholder="e.g. Container gardening for beginners"
            />
          </Field>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            <Field label="When">
              <TextInput value={f.when} onChange={set("when")} placeholder="Thu · Jun 19 · 6pm" />
            </Field>
            <Field label="Where">
              <TextInput value={f.place} onChange={set("place")} placeholder="Maple Kitchen Co-op" />
            </Field>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            <Field label="Seats available">
              <Select value={f.seats} onChange={set("seats")} options={SEATS_OPTIONS} />
            </Field>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 10 }}>
            <Button
              ...
              icon="check"
              onClick={handlePost}
              disabled={submitting || !f.skill || !f.when || !f.place}
            >
        ...
        <div style={{ position: "sticky", top: 92 }}>
          <div style={{ fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", color: K.faint, fontWeight: 700, marginBottom: 12 }}>
            Live preview
          </div>
          <PreviewCard f={f} />
          <div
          ...
          >
            ...
            <span style={{ fontSize: 12.5, color: K.forest, fontWeight: 600, lineHeight: 1.4 }}>
              Teaching earns more karma than joining — and builds your reputation on Karma.
            </span>
          </div>
        </div>
      </div>
